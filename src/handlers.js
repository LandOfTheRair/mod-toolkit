...
export const formatMod = (mod) => {
  ...
  const { fillInProperties: droptableValidator } = requireFunc(
    './resources/content/_transformers/props/droptable',
  );
  const { fillInProperties: itemValidator } = requireFunc(
    './resources/content/_transformers/props/item',
  );
  const { fillInProperties: npcScriptValidator } = requireFunc(
    './resources/content/_transformers/props/npc-scripts',
  );
  const { fillInProperties: npcValidator } = requireFunc(
    './resources/content/_transformers/props/npc',
  );
  const { fillInProperties: questValidator } = requireFunc(
    './resources/content/_transformers/props/quest',
  );
  const { fillInProperties: recipeValidator } = requireFunc(
    './resources/content/_transformers/props/recipe',
  );
  const { fillInProperties: spawnerValidator } = requireFunc(
