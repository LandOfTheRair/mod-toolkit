...
import { clone, isUndefined, objectPosition } from '../helpers';
...
export default {
  ...
  methods: {
    ...
    isValidItem(item) {
      if (
        this.items.some(
          (check, i) => i !== this.isEditing && item.name === check.name,
        )
      )
        return false;
    ...
    addContainedItem() {
      this.item.containedItems.push({
        result: `Item ${this.item.containedItems.length + 1}`,
        chance: 1,
      });
    },
