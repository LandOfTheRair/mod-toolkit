                  ...
                  <div class="row">
                    ...
                    <div class="col-4">
                      <b-button
                        variant="primary"
                        :disabled="
                          !currentTrait || npc.traitLevels[currentTrait] > 0
                        "
...
export default {
  ...
  methods: {
    ...
    isValidNPC(npc) {
      if (
        this.npcs.some(
          (check, i) => i !== this.isEditing && npc.npcId === check.npcId,
        )
      )
        return false;
    ...
    confirm() {
      events.$emit(`${this.isEditing >= 0 ? 'edit' : 'add'}:npc`, {
        npc: this.npc,
    ...
    async remove(npc) {
      const willRemove = await this.$dialog.confirm({
        title: 'Remove NPC?',
        text: 'Are you sure you want to remove this NPC?',
      });
