...
export default {
  ...
  methods: {
    ...
    isValidQuest(quest) {
      if (
        this.quests.some(
          (check, i) => i !== this.isEditing && quest.name === check.name,
        )
      )
        return false;
    ...
    async remove(quest) {
      const willRemove = await this.$dialog.confirm({
        title: 'Remove Quest?',
        text: 'Are you sure you want to remove this quest?',
      });
    ...
    makeSureDailyRepeatableSet(flag) {
      if (flag === 'daily' && this.quest.isDaily)
        this.quest.isRepeatable = true;
      if (flag === 'repeat' && !this.quest.isRepeatable)
        this.quest.isDaily = false;
    },
    ...
    formatRewards(rewards) {
      return rewards
        .map((x) => {
          ...
          if (x.type === 'stat')
            return `${x.statName} +${x.value.toLocaleString()}`;
